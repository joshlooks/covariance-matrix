...
for i in range(num_sims):
    ...
    Is[i,:,:] = np.array([np.interp(times,event_times,It[:,j]) for j in range(3)])
    Ss[i,:,:] = np.array([np.interp(times,event_times,St[:,j]) for j in range(3)])

    idx = np.searchsorted(event_times, times, side='right') - 1
    idx = np.clip(idx, 0, len(event_times) - 1) 
    Cs[i, :, :] = Ct[idx, :].T

    idx = np.searchsorted(event_times, target_days, side='right') - 1
    idx = np.clip(idx, 0, len(event_times) - 1) 
    Cs_days[i, :, :] = Ct[idx, :].T
...
fpath_C_days = os.path.join(results_dir, f'C_days_{num}_{file_id}.npy')

np.save(fpath_I, Is)
np.save(fpath_S, Ss)
np.save(fpath_C, Cs)
np.save(fpath_C_days, Cs_days)